...
func (c *config) F_webserver() (err error, ret int64) {
	/*
			...
			if c.UpgradeAerospike.HeartbeatMode == "mcast" || c.UpgradeAerospike.HeartbeatMode == "multicast" {
				...
				err = errors.New(fmt.Sprintf(ERR_HEARTBEAT_MODE, c.UpgradeAerospike.HeartbeatMode))
				ret = E_MAKECLUSTER_VALIDATION
				return err, ret
			}
	...
	*/
	return errors.New("NOT IMPLEMENTED YET"), 1
}
