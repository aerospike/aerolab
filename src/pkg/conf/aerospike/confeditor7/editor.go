	...
	menuItems = []menuItem{
			...
			Children: []menuItem{
				{
					...
					Children: []menuItem{
						{
							Type:  typeMenuItemCheckbox,
							Label: "data also on disk",
							Item:  itemStorageEngineDeviceAndMemory,
						},
						{
							Type:  typeMenuItemCheckbox,
							Label: "encryption at rest",
							Item:  itemStorageEngineEncryptionMem,
						},
						{
							Type:  typeMenuItemCheckbox,
							Label: "compression",
							Item:  itemStorageEngineCompressionMem,
						},
					},
				},
				{
		...
		},
		{
			...
			Children: []menuItem{
				{
					Type:  typeMenuItemCheckbox,
					Label: "vector metadata namespace",
...
func selectMenuItems(items []menuItem, aeroConfig aeroconf.Stanza) ([]menuItem, error) {
	...
	for i, item := range items {
		...
		case itemStorageEngineDeviceAndMemory:
			val, err := aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").GetValues("file")
			if err != nil {
				retErr = err
			}
			if err == nil && len(val) > 0 && val[0] != nil {
		...
		case itemStorageEngineEncryptionMem:
			val, err := aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").GetValues("encryption-key-file")
			if err != nil {
				retErr = err
			}
			if err == nil && len(val) > 0 && val[0] != nil {
		...
		case itemStorageEngineCompressionMem:
			val, err := aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").GetValues("compression")
			if err != nil {
				retErr = err
			}
			if err == nil && len(val) > 0 && val[0] != nil && *val[0] != "none" {
		...
		case itemOtherVectorDisk:
			if aeroConfig.Type("namespace avs-meta") != aeroconf.ValueNil && aeroConfig.Stanza("namespace avs-meta").Type("storage-engine device") != aeroconf.ValueNil {
				items[i].Selected = true
			}
		}
...
func (e *Editor) ui(v *gocui.View, key gocui.Key, ch rune, mod gocui.Modifier) {
	...
	case key == gocui.KeySpace, key == gocui.KeyEnter:
		...
		for _, change := range changes {
			...
			case itemStorageEngineEncryptionMem:
				if change.Selected {
					aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").SetValue("encryption-key-file", "/opt/aerospike/key.dat")
				} else {
					aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").Delete("encryption-key-file")
				}
			case itemStorageEngineCompressionMem:
				if change.Selected {
					aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").SetValue("compression", "zstd # alt options: none, snappy, lz4")
					aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").SetValue("compression-level", "1")
				} else {
					aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").Delete("compression")
					aeroConfig.Stanza("namespace test").Stanza("storage-engine memory").Delete("compression-level")
				}
			case itemStorageEngineAllFlash:
				if change.Selected {
			...
			case itemOtherVector:
				...
				if change.Selected {
					if aeroConfig.Type("namespace avs-meta") == aeroconf.ValueNil {
						...
						aeroConfig.Stanza("namespace avs-meta").NewStanza("storage-engine memory")
						aeroConfig.Stanza("namespace avs-meta").Stanza("storage-engine memory").SetValue("data-size", "4G")
					}
				}
			case itemOtherVectorDisk:
				for _, key := range aeroConfig.Stanza("namespace avs-meta").ListKeys() {
					if strings.HasPrefix(key, "storage-engine ") {
						aeroConfig.Stanza("namespace avs-meta").Delete(key)
					}
				}
				if change.Selected {
					aeroConfig.Stanza("namespace avs-meta").NewStanza("storage-engine device")
					aeroConfig.Stanza("namespace avs-meta").Stanza("storage-engine device").SetValue("file", "/opt/aerospike/data/avs-meta.dat")
					aeroConfig.Stanza("namespace avs-meta").Stanza("storage-engine device").SetValue("filesize", "4G")
				} else {
					aeroConfig.Stanza("namespace avs-meta").NewStanza("storage-engine memory")
					aeroConfig.Stanza("namespace avs-meta").Stanza("storage-engine memory").SetValue("data-size", "4G")
				}
