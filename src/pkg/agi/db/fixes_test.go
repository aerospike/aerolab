...
// TestDeleteBelowAgesOut exercises the age-out primitive: populate a
// time-indexed set with rows, delete everything below a cutoff, and
// verify that both the data and the index are consistent afterwards.
func TestDeleteBelowAgesOut(t *testing.T) {
	d := openTestDB(t)
	...
	const n = 500
	for i := int64(0); i < n; i++ {
		if err := d.Put("metrics", fmt.Sprintf("k%04d", i), Row{"ts": Int(i), "v": Int(i * 10)}); err != nil {
			t.Fatal(err)
		}
	}

	// Delete everything with ts < 300. Expect 300 deletions.
	got, err := d.DeleteBelow(context.Background(), "metrics", 300)
	if err != nil {
		t.Fatalf("DeleteBelow: %v", err)
	}
	if got != 300 {
		t.Errorf("DeleteBelow returned %d, want 300", got)
	}

	// Verify via Scan that the surviving rows are exactly ts=[300,500).
	it := d.Scan("metrics")
	defer it.Close()
	survivors := 0
	for it.Next() {
		...
		ts, _ := r["ts"].AsInt()
		if ts < 300 || ts >= n {
			t.Errorf("unexpected ts=%d survived", ts)
		}
		survivors++
	}
	if it.Err() != nil {
	...
	if survivors != n-300 {
		t.Errorf("survivors=%d, want %d", survivors, n-300)
	}

	// Verify the index is also consistent: a Between covering the deleted
	// range must return zero hits.
	qit := d.Query("metrics").Between("ts", Int(0), Int(299)).Run(context.Background())
	defer qit.Close()
	for qit.Next() {
		_, r := qit.Record()
		t.Errorf("stale index entry: %v", r)
	}
	if qit.Err() != nil {
		t.Fatal(qit.Err())
	}
	...
	if got != 0 {
		t.Errorf("second DeleteBelow deleted %d rows, want 0", got)
	}
}

// TestDeleteBelowUnindexedRejected verifies we don't silently scan every
// row on a set without an indexed column.
func TestDeleteBelowUnindexedRejected(t *testing.T) {
	d := openTestDB(t)
	if err := d.Put("noidx", "k1", Row{"v": Int(1)}); err != nil {
		t.Fatal(err)
	}
	if _, err := d.DeleteBelow(context.Background(), "noidx", 1); !errors.Is(err, ErrSetNotIndexed) {
		t.Errorf("want ErrSetNotIndexed, got %v", err)
	}
}

// TestDeleteBelowMissingSetNoop returns 0, nil rather than erroring.
func TestDeleteBelowMissingSetNoop(t *testing.T) {
	d := openTestDB(t)
	n, err := d.DeleteBelow(context.Background(), "never", 100)
	if err != nil {
		t.Errorf("missing-set DeleteBelow should be a no-op, got err=%v", err)
	}
	if n != 0 {
		t.Errorf("missing-set DeleteBelow returned %d, want 0", n)
	}
}
...
// TestDeleteBelowDoesNotLoseConcurrentPut exercises the lost-write race
// closed by Fix 1: a Put that lands while a chunk is being staged must
// either survive (its new ts >= hi keeps it out of the current sweep)
// or be cleanly age-out deleted. The post-sweep state must never show a
// stale row that has already been deleted according to the write log.
func TestDeleteBelowDoesNotLoseConcurrentPut(t *testing.T) {
	d := openTestDB(t)
	const set = "m"
	if err := d.RegisterSet(set, []ColumnSpec{
		{Name: "ts", Type: TypeInt64, Indexed: true},
		{Name: "v", Type: TypeInt64},
	}); err != nil {
		t.Fatal(err)
	}
	// Seed below the hi threshold so the sweep has work to do.
	const keys = 2000
	const hi int64 = 1_000_000
	for i := 0; i < keys; i++ {
		if err := d.Put(set, fmt.Sprintf("k%d", i), Row{"ts": Int(int64(i)), "v": Int(int64(i))}); err != nil {
			t.Fatal(err)
		}
	}

	// Writers hammer a subset of PKs with ts values ABOVE hi so each
	// write should survive the sweep.
	stop := make(chan struct{})
	...
	for w := 0; w < 16; w++ {
		...
		go func(id int) {
			...
			for {
				...
				pk := fmt.Sprintf("k%d", int(i)%keys)
				newTS := hi + 1 + int64(id)*1_000_000 + i
				if err := d.Put(set, pk, Row{"ts": Int(newTS), "v": Int(int64(id))}); err != nil {
	...
	n, err := d.DeleteBelow(context.Background(), set, hi)
	close(stop)
	writerWG.Wait()
	if err != nil {
		t.Fatalf("DeleteBelow: %v", err)
	}
	t.Logf("DeleteBelow removed %d rows", n)

	// Any surviving rows must have ts >= hi. A row whose current ts is
	// below hi would be a clear sweep-miss; a row that reappears
	// because a racing Put "won" and then got clobbered would surface
	// as a missing row for a key we still expect to be present. We
	// assert the former invariant globally via a full scan.
	it := d.Scan(set)
	defer it.Close()
	for it.Next() {
		k, row := it.Record()
		ts, ok := row["ts"].AsInt()
		if !ok {
			t.Errorf("row %q missing ts", k)
			continue
		}
		if ts < hi {
			t.Errorf("row %q survived DeleteBelow with ts=%d (< hi=%d)", k, ts, hi)
		}
	}
