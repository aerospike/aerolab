...
env GOOS=linux GOARCH=amd64 go build -gcflags=-trimpath=${MPATH} -asmflags=-trimpath=${MPATH} -ldflags="-s -w" -o aerolab-linux-amd64-wip || exit 1
env GOOS=linux GOARCH=arm64 go build -gcflags=-trimpath=${MPATH} -asmflags=-trimpath=${MPATH} -ldflags="-s -w" -o aerolab-linux-arm64-wip || exit 1
...
var nLinuxBinaryArm64 = ""
EOF
env GOOS=linux GOARCH=arm64 go build -gcflags=-trimpath=${MPATH} -asmflags=-trimpath=${MPATH} -ldflags="-s -w" -o aerolab-linux-arm64 || exit 1
