...
func (c *config) F_deleteData4() (ret int64, err error) {

	if c.DeleteData.RunDirect == 1 {
		return c.F_deleteData_real4()
	}
...
func (c *config) F_deleteData_real4() (ret int64, err error) {
	...
	for i := c.DeleteData.PkStartNumber; i <= c.DeleteData.PkEndNumber; i++ {
		...
		if c.DeleteData.UseMultiThreaded == 0 {
			ret, err = c.F_deleteData_perform4(i, client, wp)
			if err != nil {
...
func (c *config) F_deleteData_perform4(i int, client *aerospike.Client, wp *aerospike.WritePolicy) (ret int64, err error) {
