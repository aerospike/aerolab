...
func (ssh_client *SSH) Connect(mode int) error {
	...
	if mode == CERT_PASSWORD {
		...
		return errors.New(fmt.Sprintf("Mode not supported: %d", mode))
	}
...
func scpA(user string, addr string, privateKey string, files []fileList) error {
	...
	for _, file := range files {
		...
		errorsA := make(chan error, 10)

		cmd := shellquote.Join("scp", "-qt", path.Dir(file.filePath))
		...
		go func() {
			errorsA <- session.Wait()
			e := <-errorsA
			fmt.Println(e)
		}()

		_, _ = fmt.Fprintf(w, "C0755 %d %s\n", len(file.fileContents), path.Base(file.filePath))
