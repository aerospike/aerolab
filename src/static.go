...
var aerospikeInstallScript = map[string]string{
	"ubuntu": `export DEBIAN_FRONTEND=noninteractive; apt-get update && apt-get -y install python3-distutils; apt-get -y install libcurl4; apt-get -y install ldap-utils; apt-get -y install python3-setuptools; apt-get update && apt-get -y install iptables wget dnsutils tcpdump python net-tools vim binutils iproute2 python3 libcurl4-openssl-dev less && wget https://github.com/bestmethod/tcconfig-ubuntu-venv/archive/tc.tar.gz && tar -zxvf tc.tar.gz && mv tcconfig-ubuntu-venv-tc /tcconfig && cd /root && tar -zxf installer.tgz && cd aerospike-server-* && ./asinstall && apt-get -y install libldap-2.4-2; apt -y --fix-broken install; apt-get -y install libldap-2.4-2; cat <<'EOF' > /etc/apt/apt.conf.d/local
Dpkg::Options {
        "--force-confdef";
        "--force-confold";
}
