...
func (c *config) F_xdrConnect() (ret int64, err error) {
	...
	if inArray(clusterList, c.XdrConnect.SourceClusterName) == -1 {
		err = fmt.Errorf("Cluster does not exist: %s", c.XdrConnect.SourceClusterName)
		ret = E_BACKEND_ERROR
		return ret, err
	}
	...
	for _, destination := range destinations {
		...
		if len(destNodes) != len(destIps) {
			return E_MAKECLUSTER_NODEIPS, fmt.Errorf("Cluster %s is not on or IP allocation failed. Run cluster-list.", destination)
		}
...
`
	...
	for _, snode := range sourceNodeList {
		...
		if err != nil {
			return 999, fmt.Errorf("Failed running cat /etc/aerospike/aerospike.conf: %s", err)
		}
		conf := string(out[0])
		...
		if err != nil {
			return 999, fmt.Errorf("ERROR trying to modify config file while configuring xdr: %s\n", err)
		}
	}
