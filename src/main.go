...
func (c *config) parseCommandLineParameters(startOffset int, ignoreOffsets []int) (commandOffset int, err error) {
	...
	for i := startOffset; i < len(os.Args); i++ {
		...
		if param == "--" {
			...
			v := ""
			if len(p) > 2 {
				err = errors.New(fmt.Sprintf(ERR_MAIN_INVALIDPARAM, param))
			} else if len(p) == 2 {
				v = p[1]
			}
			if p[0] == "config" {
			...
			if i == len(os.Args) && ntype != "bool" {
				return -1, makeError("Parameter is missing argument: %s", os.Args[i-1])
			}
			if i == len(os.Args) {
...
func (c *config) parseCommandLineParametersSwitch(paramType string, param string, value string) (err error) {
	...
	if found == false {
		err = errors.New(fmt.Sprintf(ERR_MAIN_UNKNOWN_PARAM, param))
	}
	return
}
...
func (c *config) runCommand() (err error, ret int64) {
	if c.Command == "" {
