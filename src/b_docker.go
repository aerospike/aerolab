...
func (b b_docker) DeployTemplate(v version, script string, files []fileList) error {
	...
	if err != nil {
		return fmt.Errorf("Error on step 1: %s;%s", out, err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error on step 2: %s", err)
	}
	// 3.run script
	//fmt.Printf("Step 3.1: ")
	if b.host == "" {
	...
	if err != nil {
		return fmt.Errorf("Error on step 3.1: %s;%s", out, err)
	}
	//fmt.Printf("Step 3.2: ")
	if b.host == "" {
	...
	if err != nil {
		return fmt.Errorf("Error on step 3.2: %s;%s", out, err)
	}
	// 4.stop container
	//fmt.Printf("Step 4: ")
	if b.host == "" {
	...
	if err != nil {
		return fmt.Errorf("Error on step 4: %s;%s", out, err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error on step 5: %s;%s", out, err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error on step 6: %s;%s", out, err)
	}
	return nil
}
...
func (b b_docker) CopyFilesToCluster(name string, files []fileList, nodes []int) error {
	...
	for _, file := range files {
		...
		if b.host == "" {
			...
			if err != nil {
				return fmt.Errorf("Error coyping file over scp: %s", err)
			}
		}
		...
		if b.host == "" {
			...
			if err != nil {
				return fmt.Errorf("Error remote rm tmpfile: %s", err)
			}
		}
	}
	return err
	...
	for _, node := range nodes {
		...
		for _, command := range commands {
			...
			if checkExecRetcode(err) != 0 {
				return fout, fmt.Errorf("ERROR running %s: %s\n", command, err)
			}
		}
	}
	return fout, nil
...
func (b b_docker) Init() (backend, error) {
	...
	if b.host == "" {
		...
		_, err = remoteRun(b.user, b.host, b.pubkey, "/bin/bash -c 'command -v docker'")
	}
	if err != nil {
		if b.host == "" {
		...
		if err != nil {
			return b, fmt.Errorf("Could not determine platform with uname: %s, %s", err, string(out))
		}
		if strings.Contains(string(out), "Darwin") {
			...
			for _, command := range commands {
				...
				if err != nil {
					return b, fmt.Errorf("Could not install docker: %s, %s", err, string(out))
				}
			}
		}
	}
	...
	if err != nil {
		return b, fmt.Errorf("Error: docker command exists, but docker appears to be stopped. Got this:\n%s\n%s", string(out), err)
	}
	return b, nil
}
...
func (b b_docker) copyFilesToContainer(name string, files []fileList) error {
	...
	for _, file := range files {
		...
		if b.host == "" {
			...
			err = scp(b.user, b.host, b.pubkey, []fileList{fileList{fpath, file.fileContents}})
			if err != nil {
				return fmt.Errorf("Error copying files over scp: %s", err)
			}
		}
		nodeName := name
		if b.host == "" {
		...
		if b.host == "" {
			...
			_, err = remoteRun(b.user, b.host, b.pubkey, fmt.Sprintf("rm %s", fpath))
			if err != nil {
				return fmt.Errorf("Error running remote rm: %s", err)
			}
		}
	}
	return err
