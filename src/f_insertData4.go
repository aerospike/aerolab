...
func (c *config) F_insertData4() (ret int64, err error) {

	if c.InsertData.RunDirect == 1 {
		return c.F_insertData_real4()
	}
...
func (c *config) F_insertData_real4() (ret int64, err error) {
	...
	for i := c.InsertData.PkStartNumber; i <= c.InsertData.PkEndNumber; i++ {
		...
		if c.InsertData.UseMultiThreaded == 0 {
			ret, err = c.F_insertData_perform4(i, client, wp, partNo)
			if err != nil {
...
func (c *config) F_insertData_perform4(i int, client *aerospike.Client, wp *aerospike.WritePolicy, partitionNumber int) (ret int64, err error) {
