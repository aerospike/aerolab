...
type ClientListCmd struct {
	...
	Owner      string   `short:"O" long:"owner" description:"Filter by owner of the client"`
	Help       HelpCmd  `command:"help" subcommands-optional:"true" description:"Print help"`
}

func (c *ClientListCmd) Execute(args []string) error {
	cmd := []string{"client", "list"}
	...
	err = c.ListClients(system, system.Backend.GetInventory(), args, os.Stdout, nil)
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
...
func (c *ClientListCmd) ListClients(system *System, inventory *backends.Inventory, args []string, out io.Writer, page *pager.Pager) error {
	if system == nil {
		var err error
		system, err = Initialize(&Init{InitBackend: true, ExistingInventory: inventory}, []string{"client", "list"}, c, args...)
		if err != nil {
			return err
		}
	}
	...
	inventory.Instances = inventory.Instances.WithNotState(backends.LifeCycleStateTerminated).WithTags(map[string]string{"aerolab.old.type": "client"}).Describe()
