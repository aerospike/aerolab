...
func (c *config) F_netBlockUnblock(call string) (err error, ret int64) {
	// get backend
...
func (c *config) F_netBlock() (err error, ret int64) {
	return c.F_netBlockUnblock("block")
}

func (c *config) F_netUnblock() (err error, ret int64) {
	return c.F_netBlockUnblock("unblock")
}

func (c *config) F_netList() (err error, ret int64) {
	...
	for _, cluster := range clusters {
		for node, _ := range nodes[cluster] {
	...
	if strings.HasPrefix(check_out, "\n") {
