...
func (c *config) F_genTlsCerts() (err error, ret int64) {

	err, ret = chDir(c.GenTlsCerts.ChDir)
	if err != nil {
		return err, ret
	}
	...
	if c.GenTlsCerts.Nodes == "" {
		...
		for _, i := range nodes {
			if inArray(nodeList, i) == -1 {
				ret = E_BACKEND_ERROR
				return errors.New(fmt.Sprintf("Node %d does not exist", i)), ret
			}
		}
	}
	...
	if err != nil {
		return makeError("Could not mkdir ssl location: %s", err), 1
	}
	...
	for _, node := range nodes {
		...
		if strings.Contains(string(conf[0]), "mode mesh") {
			...
			for scanner.Scan() {
				t := scanner.Text()
				t = strings.Trim(t, "\r")
				if strings.Contains(t, "port 3002") && strings.Contains(t, "tls-port") == false {
