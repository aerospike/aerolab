...
func (b b_aws) Init() (backend, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Could not establish a session to aws, make sure your ~/.aws/credentials file is correct\n%s", err)
	}
...
func (b b_aws) DeployCluster(v version, name string, nodeCount int, exposePorts []string) error {
	...
	for i := start; i < (nodeCount + start); i++ {
		...
		for ni, device := range b.disks {
			...
			if err != nil {
				return fmt.Errorf("Could not convert device size to int\n%s", err)
			}
			vsize64 := int64(vsize)
			...
			if ni == 0 {
				...
				dn = fmt.Sprintf("/dev/xvd%s", string(rune('a'+ni-1)))
			}
			bdm.DeviceName = &dn
