...
// TODO make this optionally configurable or change to 8185 or something like that
...
func (c *clientCreateVectorCmd) Execute(args []string) error {
	...
	if a.opts.Config.Backend.Type == "docker" && !strings.Contains(c.Docker.ExposePortsToHost, ":"+vectorAccessPort) {
		if c.Docker.NoAutoExpose {
			fmt.Println("Docker backend is in use, but vector access port is not being forwarded. If using Docker Desktop, use '-e " + vectorAccessPort + ":" + vectorAccessPort + "' parameter in order to forward port " + vectorAccessPort + ". Press ENTER to continue regardless.")
			if !c.JustDoIt {
				var ignoreMe string
				fmt.Scanln(&ignoreMe)
			}
	...
	if a.opts.Config.Backend.Type == "docker" {
		...
		log.Println("Running client.create.vector")
		if c.ClientCount > 1 {
		...
		log.Println("Pulling and running dockerized aerospike-vector, this may take a while...")
		_, err = c.createBase([]string{"-v", confFile + ":/opt/aerospike-graph/aerospike-graph.properties"}, "graph")
		if err != nil {
			return err
		}
		log.Print("Done")
