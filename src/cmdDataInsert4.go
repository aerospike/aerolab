...
func (c *dataInsertCmd) insert4(args []string) error {
	...
	for i := c.PkStartNumber; i <= c.PkEndNumber; i++ {
		...
		if c.UseMultiThreaded == 0 {
			err = c.insert4do(i, client, wp, partNo, src)
			if err != nil {
...
func (c *dataInsertCmd) insert4do(i int, client *aerospike.Client, wp *aerospike.WritePolicy, partitionNumber int, src rand.Source) error {
