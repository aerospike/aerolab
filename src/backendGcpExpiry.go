...
func (d *backendGcp) expiriesSystemInstall(intervalMinutes int, deployRegion string, wg *sync.WaitGroup) {
	defer wg.Done()
	err := d.ExpiriesSystemInstall(intervalMinutes, deployRegion, "", false)
	if err != nil && err.Error() != "EXISTS" {
		log.Printf("WARNING: Failed to install the expiry system, clusters will not expire: %s", err)
	}
}

func (d *backendGcp) ExpiriesSystemInstall(intervalMinutes int, deployRegion string, awsDnsZoneId string, withEks bool) error {
	...
	deploy = append(deploy, "--runtime=go126")
