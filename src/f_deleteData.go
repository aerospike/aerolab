...
func (c *config) F_deleteData_real() (ret int64, err error) {
	...
	for i := c.DeleteData.PkStartNumber; i <= c.DeleteData.PkEndNumber; i++ {
		...
		if c.DeleteData.UseMultiThreaded == 0 {
			ret, err = c.F_deleteData_perform(i, client, wp)
			if err != nil {
...
func (c *config) F_deleteData_perform(i int, client *aerospike.Client, wp *aerospike.WritePolicy) (ret int64, err error) {
