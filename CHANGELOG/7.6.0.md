...
* WebUI: If `webCmd.allowls` returns `false`, `files download` and `logs get` need special handling for downloading files from servers via aerolab webui (reverse of file-upload implementation).
