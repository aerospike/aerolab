...
* webui: passwords in "Command Line" show and history should be obfuscated with '****' instead (ensure agi asftp/s3 passwords are of type Password and obfuscated too, along any other agi password fields)
...
* WebUI: Implement `simple` mode in which the number of items/options/features is reduced, hiding the more obscure settings.
  ...
  * in simple mode, form fields should be hidden, not removed; fields that are disabled in simple mode cannot be modified (server-side check and error); CLI command line should have "withDefaults" switch to show full command line if defaults were not set
