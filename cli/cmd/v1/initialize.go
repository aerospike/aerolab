...
func (i *Init) upgradeCheck(s *System) {
	...
	if uc.CurrentVersion == currentVersion && uc.IsUpgradeAvailable {
		log.Info("A new AeroLab version %s is available, install using `aerolab upgrade`, or download link: https://github.com/citrusleaf/aerolab/releases", uc.NewVersion)
	}
