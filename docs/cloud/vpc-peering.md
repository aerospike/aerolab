# VPC Peering for Cloud Clusters

Guide for peering VPCs with Aerospike Cloud clusters.

## Overview

VPC peering allows your AWS resources to connect to Aerospike Cloud clusters over a private network connection. This is typically done automatically during cluster creation, but you can also peer additional VPCs.

## Automatic VPC Peering

When creating a cluster, VPC peering is automatically set up with the specified VPC:
...
3. Set up VPC peering with the cluster

## Manual VPC Peering

You can also peer additional VPCs with an existing cluster:
...
| Option | Description |
|--------|-------------|
| `-d, --cluster-id` | Cluster ID (required) |
...
3. **Route** (`--stage-route`): Create route in VPC route table to direct traffic to the peered cluster
4. **Associate DNS** (`--stage-associate-dns`): Associate VPC with the cluster's private hosted zone
...
The VPC must have a valid CIDR block that doesn't conflict with the cluster's network.

### Cloud Cluster CIDR

By default, Aerospike Cloud clusters are assigned a CIDR block starting from `10.128.0.0/19`. When you specify a VPC-ID during cluster creation, aerolab automatically:
...
3. **Applies the CIDR**: Uses the available CIDR for the cluster infrastructure
...
**Note**: If you specify a custom CIDR that is already in use in your VPC routes, the command will fail with an error before creating the cluster.
...
Once VPC peering is established, you can connect to the cluster from resources in the peered VPC:
...
3. Verify peering status:
   ```bash
   aerolab cloud clusters list -o json | jq '.clusters[] | select(.name == "mydb") | .vpc'
   ```
...
This means there's already a route in your VPC's route table for the cluster's CIDR block, but it points to a different destination (e.g., another peering connection, NAT gateway, or internet gateway).
...
2. **CIDR Blocks**: Ensure VPC CIDR blocks don't conflict with the cloud cluster CIDR
3. **Auto CIDR Resolution**: When creating clusters with `--vpc-id`, aerolab automatically detects CIDR collisions and finds available CIDRs
4. **Custom CIDR**: Use `--cloud-cidr` during cluster creation to specify a custom CIDR block
...
7. **Multiple VPCs**: You can peer multiple VPCs with the same cluster
