...
openssl x509 -req -extfile openssl.conf -in input/server1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/server1.pem -set_serial 110
openssl x509 -req -extfile openssl.conf -in input/client1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/client1.pem -set_serial 210
openssl x509 -req -extfile openssl.conf -in input/admin1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/admin1.pem -set_serial 310
openssl x509 -req -extfile openssl.conf -in input/ldap1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/ldap1.pem -set_serial 410
openssl x509 -req -extfile openssl.conf -in input/xdr1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/xdr1.pem -set_serial 510
openssl x509 -req -extfile openssl.conf -in input/fabric1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/fabric1.pem -set_serial 610
openssl x509 -req -extfile openssl.conf -in input/heartbeat1.req -CA local/rootCA.pem   -CAkey local/rootCA.key  -extensions v3_req -days 3649 -outform PEM -out output/heartbeat1.pem -set_serial 710

echo
echo "Verify Certificate signed by root"
openssl verify -verbose -CAfile local/rootCA.pem output/server1.pem
openssl verify -verbose -CAfile local/rootCA.pem output/client1.pem
openssl verify -verbose -CAfile local/rootCA.pem output/admin1.pem
openssl verify -verbose -CAfile local/rootCA.pem output/ldap1.pem
openssl verify -verbose -CAfile local/rootCA.pem output/xdr1.pem
openssl verify -verbose -CAfile local/rootCA.pem output/fabric1.pem
openssl verify -verbose -CAfile local/rootCA.pem output/heartbeat1.pem
